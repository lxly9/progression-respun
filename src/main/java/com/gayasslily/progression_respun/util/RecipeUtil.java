...
import com.gayasslily.progression_respun.recipe.GrindingRecipe;
import com.gayasslily.progression_respun.recipe.GrindingRecipeInput;
import com.gayasslily.progression_respun.recipe.ModRecipes;
import com.gayasslily.progression_respun.recipe.*;
...
public class RecipeUtil {
    ...
    public static void applyRecipeFilter(MinecraftServer server) {
        ...
        for (RecipeEntry<?> entry : allRecipes) {
            ...
            boolean isSmeltType = recipeType.equals(RecipeType.SMELTING);
            boolean idHasOre = identifier.getPath().matches(".*(ore|copper|iron|gold|silver|netherite).*");

            if (isSmeltType && (idHasOre || identifier.getPath().equals("quartz"))) continue;
            if (recipe instanceof ShapedRecipe && identifier.getPath().equals("furnace")) continue;
